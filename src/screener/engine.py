...
def calculate_composite_score(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Normalize metrics within sector using P10/P90 winsorisation
    ...
    # Sector-relative composite score computation
        ...
        if metric.endswith("_min"):
            ...
            if col in filtered.columns:
                if col == "interest_coverage":
                    # ICR filter: Debt Free or None always passes
            ...
            if col in filtered.columns:
                if col == "debt_to_equity":
                    # Skip D/E max filter for Financials sector
                    is_fin = filtered["broad_sector"].str.upper().isin(FINANCIAL_SECTORS)
...
def export_screener_excel(preset_results: Dict[str, pd.DataFrame], output_path: str = "output/screener_output.xlsx"):
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        for preset_name, df in preset_results.items():
            ...
            # Apply cell highlight styling for core score
