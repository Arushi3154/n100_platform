...
class ScreenerEngine:
    ...
    def apply_icr_and_de_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "icr" in df.columns:
            df["icr_numeric"] = df["icr"].apply(
                lambda x: (
                    np.inf
                    if str(x).strip().lower() == "debt free"
    ...
    def compute_composite_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            "fcf" in df.columns
            and "cash_quality" in weights
            and "fcf_positive" in weights["cash_quality"]
        ):
            score += (
                (df["fcf"] > 0).astype(int)
                * 100
                * weights["cash_quality"]["fcf_positive"]
            )
    ...
    def run_preset(self, df: pd.DataFrame, preset_name: str) -> pd.DataFrame:
        ...
        for metric, threshold in preset.items():
            if metric == "de_max" and "de" in filtered.columns:
                ...
                metric == "revenue_cagr_5yr_min"
                and "revenue_cagr_5yr" in filtered.columns
            ):
                filtered = filtered[filtered["revenue_cagr_5yr"] >= threshold]
            elif (
                metric == "revenue_cagr_3yr_min"
                and "revenue_cagr_3yr" in filtered.columns
            ):
                ...
                metric == "dividend_yield_min" and "dividend_yield" in filtered.columns
            ):
                filtered = filtered[filtered["dividend_yield"] >= threshold]
            elif (
                metric == "dividend_payout_max"
                and "dividend_payout" in filtered.columns
            ):
                ...
                metric == "asset_turnover_min" and "asset_turnover" in filtered.columns
            ):
